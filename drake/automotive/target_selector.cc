...
TargetSelector<T>::TargetSelector(
  ...
  // First, declare a  port for the self car.
  // this->DeclareInputPort(systems::kVectorValued, 2,
  //                       systems::kContinuousSampling);
  this->DeclareAbstractOutputPort(systems::kContinuousSampling);
  // Next, declare a port for each of the M target traffic cars of interest.
...
template <typename T>
const systems::SystemPortDescriptor<T>& TargetSelector<T>::get_self_inport()
    const {
  return systems::System<T>::get_input_port(0);
}

template <typename T>
const systems::SystemPortDescriptor<T>& TargetSelector<T>::get_world_inport(
    const int i) const {
...
template <typename T>
const systems::SystemPortDescriptor<T>& TargetSelector<T>::get_self_outport()
    const {
  return systems::System<T>::get_output_port(0);
}

template <typename T>
const systems::SystemPortDescriptor<T>& TargetSelector<T>::get_target_outport(
    const int i) const {
...
void TargetSelector<T>::EvalOutput(const systems::Context<T>& context,
  ...
  systems::AbstractValue* const output_data_self = output->GetMutableData(0);
  CarData& output_self = output_data_self->GetMutableValue<CarData>();
  std::cerr << "TargetSelector GetMutableData (self)." << std::endl;

  std::cerr << "TargetSelector num_targets_per_car_:" << num_targets_per_car_
            << std::endl;
  ...
  SelectCarStateAndEvalOutput(basic_input_self, inputs_world, &output_self,
                              outputs_target);
  std::cerr << " ^^^^^^^^ TargetSelector::EvalOutput." << std::endl;
}
...
void TargetSelector<T>::SelectCarStateAndEvalOutput(
    ...
    CarData* output_self, std::vector<CarData*> outputs_target) const {
  ...
  for (int i = 0; i < num_cars_; ++i) {
    ...
    if (!do_restrict_to_lane_) {
      ...
      if (i == 0) {
        ...
        CarData car_data_infinite(position_enormous, 0., rp.lane);
        ...
        output_self->set(rp_pos, longitudinal_speed, rp.lane);
    ...
    if (i > 0 && distances[i] < kPerceptionDistance) {
      ...
      CarData car_data_value(rp_pos, longitudinal_speed, rp.lane);
...
template <typename T>
std::vector<int> TargetSelector<T>::SortDistances(
