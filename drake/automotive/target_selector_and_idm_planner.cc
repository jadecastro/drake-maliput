#include "target_selector_and_idm_planner.h"
...
TargetSelectorAndIdmPlanner<T>::get_self_inport() const {
  return systems::System<T>::get_input_port(0);
}

template <typename T>
const systems::SystemPortDescriptor<T>&
TargetSelectorAndIdmPlanner<T>::get_world_inport(const int i) const {
  DRAKE_DEMAND(i < num_cars_ - 1);
  return systems::System<T>::get_input_port(i + 1);
}

template <typename T>
void TargetSelectorAndIdmPlanner<T>::EvalOutput(
...
template <typename T>
std::pair<double, double> TargetSelectorAndIdmPlanner<T>::AssessLongitudinal(
  ...
  while (lane_this_car != path_self_car.end()) {
    ...
    if (is_car_past_limit) {
      ...
      bool is_not_reversed_nor_infty = !lane_this_car->is_reversed ||
          (rp_this_car.pos.s > 0.5*kEnormousDistance);
      const double pos_relative_to_lane = (is_not_reversed_nor_infty) ?
...
template <typename T>
std::vector<int> TargetSelectorAndIdmPlanner<T>::SortDistances(
...
TargetSelectorAndIdmPlanner<T>::AllocateParameters() const {
...
template class TargetSelectorAndIdmPlanner<double>;
